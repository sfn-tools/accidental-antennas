    ...
    **transform_kwargs,
    ...
    for device in objects.devices:
        ...
        if pec_mask is not None:
            ordered_materials = compute_ordered_materials(device.materials)
            pec_flags = [m.is_pec for m in ordered_materials]
            if device.output_type == ParameterType.CONTINUOUS:
                if len(pec_flags) == 2 and any(pec_flags):
                    if pec_flags[0] and pec_flags[1]:
                        base = jnp.ones_like(cur_material_indices)
                    elif pec_flags[0]:
                        base = 1.0 - cur_material_indices
                    else:
                        base = cur_material_indices
                    hard = (base > 0.5).astype(cur_material_indices.dtype)
                    pec_slice = straight_through_estimator(base, hard)
                else:
                    pec_slice = jnp.zeros_like(cur_material_indices)
            else:
                pec_flags_jnp = jnp.asarray(pec_flags, dtype=cur_material_indices.dtype)
                pec_slice = pec_flags_jnp[cur_material_indices.astype(jnp.int32)]
            pec_mask = pec_mask.at[*device.grid_slice].set(pec_slice)
    ...
    if pec_mask is not None:
        arrays = arrays.at["pec_mask"].set(pec_mask)
...
def _init_arrays(
    ...
    # PEC mask for perfect conductors
    pec_mask = create_named_sharded_matrix(
        volume_shape,
        value=0.0,
        dtype=config.dtype,
        sharding_axis=1,
    ...
    for o in sorted_obj:
        if isinstance(o, UniformMaterialObject):
            ...
            if o.material.is_pec:
                pec_mask = pec_mask.at[*o.grid_slice].set(1.0)
            ...
            ordered_materials = compute_ordered_materials(o.materials)
            pec_flags = jnp.asarray([1.0 if m.is_pec else 0.0 for m in ordered_materials], dtype=config.dtype)
            pec_indices = pec_flags[indices]
            pec_slice = mask.astype(config.dtype) * pec_indices
            current = pec_mask[*o.grid_slice]
            pec_mask = pec_mask.at[*o.grid_slice].set(jnp.maximum(current, pec_slice))
        else:
