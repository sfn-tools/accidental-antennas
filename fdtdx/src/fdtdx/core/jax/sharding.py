...
def get_named_sharding_from_shape(
    ...
    """
    compute_devices = devices or jax.devices()
    num_dims = len(shape)
...
def create_named_sharded_matrix(
    ...
    if shape[sharding_axis] % num_devices != 0:
        for axis, dim in enumerate(shape):
            if dim > 1 and dim % num_devices == 0:
                sharding_axis = axis
                break
    if shape[sharding_axis] % num_devices != 0:
        device = compute_devices[0]
        arr = jnp.ones(shape, dtype=dtype, device=device)
        arr = arr * value
        counter += math.prod(shape) * get_dtype_bytes(dtype)
        return arr
